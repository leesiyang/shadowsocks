...
function download_files(){
    if [ "$OS" == 'CentOS' ]; then
        ...
        if ! wget --no-check-certificate https://raw.githubusercontent.com/leesiyang/shadowsocks/master/shadowsocks -O /etc/init.d/shadowsocks; then
            echo "Failed to download shadowsocks chkconfig file!"
            exit 1
        fi
    else
        if ! wget --no-check-certificate https://raw.githubusercontent.com/leesiyang/shadowsocks/master/shadowsocks-debian -O /etc/init.d/shadowsocks; then
            echo "Failed to download shadowsocks chkconfig file!"
            exit 1
        fi
    fi
...
function install_ss(){
    ...
    if [ -f /usr/bin/pip ]; then
        ...
        if [ -f /usr/bin/ssserver ] || [ -f /usr/local/bin/ssserver ]; then
            ...
            echo
            echo "Shadowsocks install failed! Please visit https://github.com/leesiyang/shadowsocks/ and contact."
            exit 1
        fi
        clear
        echo
        ...
        echo
        echo "pip install failed! Please visit https://github.com/leesiyang/shadowsocks/ and contact."
        exit 1
    fi
}
