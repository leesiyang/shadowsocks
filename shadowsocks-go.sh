...
function download_files(){
    cd $cur_dir
    if is_64bit; then
        if ! wget -c https://raw.githubusercontent.com/leesiyang/shadowsocks/master/shadowsocks-server-linux64-1.1.5.gz;then
        ...
        if ! wget -c https://raw.githubusercontent.com/leesiyang/shadowsocks/master/shadowsocks-server-linux32-1.1.5.gz;then
    ...
    if [ "$OS" == 'CentOS' ];then
        if ! wget --no-check-certificate -O shadowsocks-go https://raw.githubusercontent.com/leesiyang/shadowsocks/master/shadowsocks-go; then
            echo "Failed to download shadowsocks-go auto start script!"
            exit 1
        fi
    else
        if ! wget --no-check-certificate -O shadowsocks-go https://raw.githubusercontent.com/leesiyang/shadowsocks/master/shadowsocks-go-debian; then
            echo "Failed to download shadowsocks-go auto start script!"
            exit 1
        fi
    fi
